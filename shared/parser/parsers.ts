...
const parseFragment = (html: string): Parse5DocumentFragment => {
...
export const parseLeadFinalsResults = (document: Parse5DocumentFragment): LeadFinalsItem[] => {
  return parseTable<LeadFinalsItem>(document, leadFinalsConfig as Array<{ parserId: number; prop: keyof LeadFinalsItem }>);
};
...
export const parseBoulderFinal = (document: Parse5DocumentFragment): BoulderFinalItem[] => {
  return parseTable<BoulderFinalItem>(document, boulderFinalsConfig as Array<{ parserId: number; prop: keyof BoulderFinalItem }>);
};
...
const parseRouteCell = (node: Parse5Node | null | undefined): string => {
  if (!node) return '';
